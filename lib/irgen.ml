...
let translate (sfunc): L.llmodule = 
  match sfunc with 
      SFuncDecl (_, _, _, sbody) ->
        ...
        let all_funcs = Hashtbl.create 1 in

        let add_func (sfunc) =
          match sfunc with 
              SFuncDecl (sfname, sparams, srtype, sbody) ->
                let param_dtypes = Array.of_list (List.map (fun (typ, _) -> lltype_of_dtype typ) sparams) in
                let ftype = L.function_type (lltype_of_dtype srtype) param_dtypes in
                Hashtbl.add all_funcs sfname (L.define_function sfname ftype mdl, (sfname, sparams, srtype, sbody))
            | _ -> ()
        in List.iter add_func sbody; add_func sfunc;

        let rec build_func_body (sfunc) = 
          match sfunc with
              SFuncDecl (sfname, sparams, srtype, sbody) ->
                

                let (the_func, _) = Hashtbl.find all_funcs sfname in
                ...
                let body_scope = Hashtbl.create (List.length sparams + List.length sbody) in
                ...
                in List.iter2 add_param sparams (Array.to_list (L.params the_func));
                ...
                let rec build_expr builder exp = match exp with
                  ...
                  | SECall (id, params) -> 
                      let (fdef, (_, _, srtype', _)) = Hashtbl.find all_funcs id in
                      let llargs = List.rev (List.map (fun item -> build_expr builder (snd item)) (List.rev params)) in
                      let res = if srtype' = None then "" else id ^ "_result" in
                ...
                let rec build_stmt builder sstmt = match sstmt with
                    SInit (typ, id, sexp) -> 
                  ...
                  | SCondLoop (prd, block) ->
                      ...
                      L.builder_at_end context merge_bb
                  | SFuncDecl (_, _, _, _) -> ignore(build_func_body (sstmt)); builder 
                  | SReturn sexp -> 
                      ignore (
                        if srtype = None then L.build_ret_void builder
                ...
                let builder = List.fold_left build_stmt builder sbody in

                if srtype = None then add_terminal builder (L.build_ret_void)
                else add_terminal builder (L.build_unreachable)
            | _ -> raise (Failure internal_err)

        in build_func_body sfunc;
        mdl
    | _ -> raise (Failure internal_err)
